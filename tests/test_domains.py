...
class TestDomainProperties:
    ...
    def test_organization_name(self, domain: Domain) -> None:
    ...
    def test_created_at_none_when_missing(self, mock_client: MagicMock) -> None:
        """created_at returns None when the createdAt field is absent."""
        d = Domain.from_payload(mock_client, {"domainId": "abc"})
        assert d.created_at is None

    def test_name_setter(self, domain: Domain) -> None:
    ...
    def test_dcv_expires_from_valid_till(self, mock_client: MagicMock) -> None:
    ...
    def test_dcv_expires_none_when_missing(self, mock_client: MagicMock) -> None:
    ...
    def test_verified_at_from_detail_response(self, mock_client: MagicMock) -> None:
    ...
    def test_verified_at_none_when_missing(self, mock_client: MagicMock) -> None:
        """verified_at returns None when verifiedAt is absent (e.g. PENDING domain)."""
        d = Domain.from_payload(mock_client, dict(SAMPLE_DOMAIN_DATA_2))
        assert d.verified_at is None

    def test_dcv_expires_soon_true_within_threshold(self, mock_client: MagicMock) -> None:
    ...
    def test_dcv_expires_soon_false_far_future(self, mock_client: MagicMock) -> None:
    ...
    def test_dcv_expires_soon_false_when_no_expiry(self, mock_client: MagicMock) -> None:
        """dcv_expires_soon returns False when dcv_expires is None."""
        d = Domain.from_payload(mock_client, dict(SAMPLE_DOMAIN_DATA_2))
        assert d.dcv_expires_soon(30) is False

    def test_missing_fields_return_none(self, mock_client: MagicMock) -> None:
...
class TestDomainDunderMethods:
    ...
    def test_print_does_not_raise(self, domain: Domain) -> None:
...
class TestDomainHelpers:
    ...
    def test_as_dict_returns_raw_data(self, domain: Domain) -> None:
    ...
    def test_to_row_keys(self, domain: Domain) -> None:
    ...
    def test_to_row_values_are_strings(self, domain: Domain) -> None:
        """to_row() returns only string values."""
        assert all(isinstance(v, str) for v in domain.to_row().values())

    def test_to_row_name(self, domain: Domain) -> None:
...
class TestDomainNeedsDcv:
    ...
    def test_active_pending_needs_dcv(self, mock_client: MagicMock) -> None:
        """ACTIVE + PENDING → needs_dcv is True."""
        assert self._domain(mock_client, "ACTIVE", "PENDING").needs_dcv is True

    def test_active_verified_does_not_need_dcv(self, mock_client: MagicMock) -> None:
        """ACTIVE + VERIFIED → needs_dcv is False."""
        assert self._domain(mock_client, "ACTIVE", "VERIFIED").needs_dcv is False

    def test_inactive_pending_does_not_need_dcv(self, mock_client: MagicMock) -> None:
    ...
    def test_active_failed_needs_dcv(self, mock_client: MagicMock) -> None:
    ...
    def test_sample_domain_data_does_not_need_dcv(self, domain: Domain) -> None:
        """The standard fixture (ACTIVE/VERIFIED) has needs_dcv False."""
        assert domain.needs_dcv is False

    def test_sample_domain_data_2_needs_dcv(self, mock_client: MagicMock) -> None:
    ...
    def test_missing_status_does_not_need_dcv(self, mock_client: MagicMock) -> None:
...
class TestDomainAPIMethods:
    ...
    def test_refresh_calls_get(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_deactivate_calls_post(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_get_dcv_calls_get(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_get_dcv_unset_returns_empty_dcvinfo(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_get_dcv_handles_fallback_dcvmethod_field(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_get_dcv_returns_empty_dcvinfo_on_bad_response(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_change_dcv_method_rejects_unknown_method(self, domain: Domain, mock_client: MagicMock) -> None:
        ...
        with pytest.raises(ValueError, match="Invalid DCV method"):
            # Deliberately outside the Literal, to exercise the runtime rejection.
    ...
    def test_reinitiate_dcv_raises_when_method_unknown(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_verify_calls_post(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_change_dcv_method_calls_patch_with_method(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_last_dcv_attempt_calls_get(self, domain: Domain, mock_client: MagicMock) -> None:
    ...
    def test_dcv_attempt_history_calls_get(self, domain: Domain, mock_client: MagicMock) -> None:
...
class TestDomainAccessorList:
    ...
    def test_returns_list_of_domain_objects(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_handles_paginated_dict_response(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_passes_offset_and_limit(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_no_params_paginates_with_sortby_domainname(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_returns_empty_list_when_no_domains(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """list returns an empty list when the API returns an empty array."""
        mock_client.get.return_value = []
        assert accessor.get_list() == []

    def test_pattern_filters_by_exact_name(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_pattern_alternation_matches_multiple(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """list(pattern=) with alternation returns all matching domains."""
        mock_client.get.return_value = [SAMPLE_DOMAIN_DATA, SAMPLE_DOMAIN_DATA_2]
        result = accessor.get_list(pattern="umaine\\.edu|maine\\.edu")
        assert len(result) == 2

    def test_pattern_is_case_insensitive(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """list(pattern=) matches regardless of case."""
        mock_client.get.return_value = [SAMPLE_DOMAIN_DATA]
        result = accessor.get_list(pattern="UMAINE\\.EDU")
        assert len(result) == 1

    def test_pattern_no_match_returns_empty(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """list(pattern=) returns an empty list when no domains match."""
        mock_client.get.return_value = [SAMPLE_DOMAIN_DATA, SAMPLE_DOMAIN_DATA_2]
        assert accessor.get_list(pattern="notfound\\.edu") == []

    def test_pattern_none_returns_all(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """list(pattern=None) returns all domains unfiltered."""
        mock_client.get.return_value = [SAMPLE_DOMAIN_DATA, SAMPLE_DOMAIN_DATA_2]
        assert len(accessor.get_list(pattern=None)) == 2

    def test_pattern_wildcard_matches_subdomain(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """list(pattern=) supports regex wildcards for subdomain matching."""
        mock_client.get.return_value = [SAMPLE_DOMAIN_DATA, SAMPLE_DOMAIN_DATA_2]
        result = accessor.get_list(pattern=r".*\.edu")
        assert len(result) == 2

    def test_search_passed_as_query_param(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_domain_status_passed_as_query_param(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_dcv_status_passed_as_query_param(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_server_side_params_combined(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
...
def _domain_row(idx: int) -> dict[str, Any]:
...
class TestDomainAccessorListPagination:
    ...
    def test_stops_on_empty_page(
        self, accessor: DomainAccessor, mock_client: MagicMock, monkeypatch: pytest.MonkeyPatch
    ) -> None:
    ...
    def test_stops_at_defensive_page_ceiling(
        self, accessor: DomainAccessor, mock_client: MagicMock, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        ...
        def _next_page(*_args: Any, **_kwargs: Any) -> list[dict[str, Any]]:
...
class TestDomainAccessorGetPendingDcv:
    ...
    def test_returns_only_pending_domains(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_returns_empty_when_all_verified(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """get_pending_dcv() returns an empty list when all domains are verified."""
        mock_client.get.return_value = [SAMPLE_DOMAIN_DATA]
        assert accessor.get_pending_dcv() == []

    def test_returns_all_when_all_pending(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_returns_empty_when_no_domains(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """get_pending_dcv() returns an empty list when there are no domains."""
        mock_client.get.return_value = []
        assert accessor.get_pending_dcv() == []

    def test_excludes_inactive_domains(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_result_contains_domain_instances(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_filters_domain_status_server_side_only(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
...
class TestDomainAccessorGet:
    ...
    def test_get_by_id_calls_endpoint_directly(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_get_by_name_searches_list(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """get by name calls list() and returns the matching domain."""
        mock_client.get.return_value = [SAMPLE_DOMAIN_DATA, SAMPLE_DOMAIN_DATA_2]
        result = accessor.get("maine.edu")
        assert result.name == "maine.edu"

    def test_get_by_name_is_case_insensitive(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
        """get by name matches regardless of case."""
        mock_client.get.return_value = [SAMPLE_DOMAIN_DATA]
        result = accessor.get("UMAINE.EDU")
        assert result.name == "umaine.edu"

    def test_get_by_name_raises_key_error_when_not_found(
...
class TestDomainAccessorCreate:
    ...
    def test_create_with_organization_id(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
...
class TestDomainAccessorDeactivate:
    ...
    def test_deactivate_posts_to_correct_url(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
    ...
    def test_deactivate_returns_domain(self, accessor: DomainAccessor, mock_client: MagicMock) -> None:
