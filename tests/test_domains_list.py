...
class TestDomainListFixture:
    ...
    def test_remaining_domains_are_pending(self, domains_list: list[Domain]) -> None:
    ...
    def test_all_domains_have_created_at(self, domains_list: list[Domain]) -> None:
        """Every domain has a non-None created_at datetime."""
        assert all(d.created_at is not None for d in domains_list)

    def test_all_created_at_are_utc(self, domains_list: list[Domain]) -> None:
    ...
    def test_all_created_at_same_date(self, domains_list: list[Domain]) -> None:
    ...
    def test_all_domains_belong_to_same_org(self, domains_list: list[Domain]) -> None:
    ...
    def test_all_domains_share_org_id(self, domains_list: list[Domain]) -> None:
        """All domains share the same organization_id."""
        org_ids = {d.organization_id for d in domains_list}
        assert len(org_ids) == 1

    def test_to_row_for_every_domain(self, domains_list: list[Domain]) -> None:
        """to_row() succeeds for every domain and returns string values."""
        for d in domains_list:
            row = d.to_row()
            assert all(isinstance(v, str) for v in row.values())

    def test_str_for_every_domain(self, domains_list: list[Domain]) -> None:
        """str() succeeds for every domain and includes the domain name."""
        for d in domains_list:
            text = str(d)
            assert d.name is not None and d.name in text

    def test_repr_for_every_domain(self, domains_list: list[Domain]) -> None:
...
class TestDomainAccessorWithFullList:
    ...
    def test_list_returns_all_43_domains(
        self, accessor: DomainAccessor, mock_client: MagicMock, domains_list_data: list[dict[str, Any]]
    ) -> None:
