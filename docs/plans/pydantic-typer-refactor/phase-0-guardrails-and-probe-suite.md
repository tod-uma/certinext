...
| R16 | Reports use 1-based `page`/`size` (max 100), short-page termination; no server-side domain/CN filter on /reports/orders → client-side `find_by_domain` | Check current OpenAPI for new filter params; verify page-size max + empty-page shape | both |
