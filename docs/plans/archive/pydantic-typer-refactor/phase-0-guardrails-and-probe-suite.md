...
| R21 | `preVettingToken`/`preVetted`, csr-in-create, `delegation`, `recipientEmails`, `tags` exist only in OpenAPI (absent from Postman docs); memory: token reportedly not applying to one prod org | Sandbox OV order with/without token (auto-approve vs manual queue); confirm fields still in sandbox OpenAPI | sandbox |
