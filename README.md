...
**Pre-releases** — to get the latest alpha/beta/rc ahead of the next stable
release, add `--prerelease=allow`:
...
**Pre-releases** — to get the latest alpha/beta/rc ahead of the next stable
release, add `--pre`/`--pip-args=--pre`:

```bash
pipx install --pip-args=--pre "certinext[csr,keyring]"
